...
flowchart TD
    ...
    subgraph AI_Engine["AI Engine (Rust Binary)"]
        BIN["polyglid-ai<br/>(.workspace/ai/rust/)"]
        CONFIG_FILE["ai-config.toml<br/>(.workspace/ai/configs/)"]
    ...
    subgraph Output["Outputs"]
        REPORTS["Reports<br/>(.workspace/ai/output/reports/)"]
...
make init
  ...
  │     └── Writes .workspace/ai/configs/ai-config.toml
...
### Primary: `.workspace/ai/configs/ai-config.toml`
...
### Per-domain model configs: `.workspace/ai/configs/model-configs/*.toml`
...
# Place a GGUF model at .workspace/ai/models/gguf/llama-2-7b.Q4_K_M.gguf
```
