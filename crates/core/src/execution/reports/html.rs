...
pub fn export(report: &ExportedReport) -> Result<String, String> {
    ...
    html.push_str(
        "h1 { border-bottom: 2px solid #e1e4e8; padding-bottom: 10px; margin-top: 0; }\n",
    );
    ...
    html.push_str(&format!(
        "<tr><th>PolyGlid Version</th><td>{}</td></tr>\n",
        report.metadata.polyglid_version
    ));
    html.push_str(&format!(
        "<tr><th>Plugin ID</th><td>{}</td></tr>\n",
        report.metadata.plugin_id
    ));
    html.push_str(&format!(
        "<tr><th>Plugin Version</th><td>{}</td></tr>\n",
    ...
    html.push_str(&format!(
        "<tr><th>Security Profile</th><td>{}</td></tr>\n",
        report.metadata.security_profile
    ));
    html.push_str(&format!(
        "<tr><th>Execution Duration</th><td>{} ms</td></tr>\n",
        report.metadata.execution_duration_ms
    ));
    html.push_str(&format!(
        "<tr><th>Report Format Version</th><td>{}</td></tr>\n",
    ...
    if report.report.issues.is_empty() {
        ...
        for issue in &report.report.issues {
            ...
            html.push_str(&format!(
                ...
                sev_class, issue.severity, issue.title, issue.description, issue.recommendation
            ));
        }
        html.push_str("</table>\n");
    }
