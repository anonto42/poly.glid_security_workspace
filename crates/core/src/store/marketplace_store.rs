...
impl MarketplaceStore {
    ...
    pub fn list_publishers(&self) -> Result<Vec<DbPublisherProfile>, String> {
        ...
        let result: Result<Vec<DbPublisherProfile>, _> = stmt
            .query_map([], |row| {
    ...
    pub fn get_publisher(&self, id: &str) -> Result<Option<DbPublisherProfile>, String> {
        ...
        let result: Result<Vec<DbPublisherProfile>, _> = stmt
            ...
            .collect();
        Ok(result
            .map_err(|e: rusqlite::Error| format!("row: {e}"))?
            .into_iter()
            .next())
    }
    ...
    fn collect_packages(
        ...
        let result: Result<Vec<DbMarketplacePackage>, _> = stmt
            .query_map(p, |row| {
    ...
    pub fn list_featured(&self) -> Result<Vec<DbMarketplacePackage>, String> {
        let conn = self.conn.lock().unwrap();
        let sql = format!(
            "SELECT {} ORDER BY is_featured DESC, download_count DESC, rating_avg DESC LIMIT 20",
            Self::PKG_COLS
        );
    ...
    pub fn increment_download_count(&self, package_id: &str) -> Result<(), String> {
        ...
        .map(|_| ())
        .map_err(|e| format!("increment_download_count: {e}"))
    }
    ...
    pub fn list_ratings(&self, package_id: &str) -> Result<Vec<DbMarketplaceRating>, String> {
        ...
        let mut stmt = conn
            ...
            .map_err(|e| format!("prepare: {e}"))?;
        let result: Result<Vec<DbMarketplaceRating>, _> = stmt
    ...
    pub fn list_installed_packages(&self) -> Result<Vec<DbMarketplaceInstall>, String> {
        ...
        let result: Result<Vec<DbMarketplaceInstall>, _> = stmt
            .query_map([], |row| {
...
mod tests {
    ...
    fn test_publish_and_search() {
        ...
        store
            .publish_package(&sample_pkg("pkg-001", "recon-probe"))
            .unwrap();
    ...
    fn test_rating_avg() {
        ...
        let store = ws.marketplace();
        store
            .publish_package(&sample_pkg("pkg-002", "dns-enum"))
            .unwrap();
        store
            .add_rating(&DbMarketplaceRating {
                id: "r1".to_string(),
    ...
    fn test_download_count() {
        ...
        let store = ws.marketplace();
        store
            .publish_package(&sample_pkg("pkg-003", "ssl-checker"))
            .unwrap();
